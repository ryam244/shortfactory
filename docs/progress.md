...
2026-09-25追記：共感を軸にした編集サンプルを調整。Midjourney背景は候補素材として登録し、人物との画風差があったため採用版ではオリジナルの統一ベクター背景へ戻した。実VOICEVOX音声7場面＋Suno WAVによる約28秒のMP4を生成し、解像度・コーデック・音声・fps・尺の技術検証に合格。画面に完成MP4の再生を追加。制作方針・参照資料・再現手順は[ショート動画の制作ベース](./short-video-creative-base.md)へ保存。実視聴者の評価と、任意テーマへの台本自動化は未完了。
...
| --- | --- | --- | --- |
| 前提：素材準備 | 一部完了・権利確認待ち | Midjourney背景候補1点、Suno WAV 1曲、オリジナルSVG10点、VOICEVOX音声7場面を登録。出所・権利メモを保存 | Midjourney/Sunoの公開・商用利用条件を確認し、画風を揃えた実ブランド素材一式を確定 |
