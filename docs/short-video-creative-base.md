...
- 通常テンプレートはSNS右側UIを避ける安全域の中央、完成シーン方式は画面中央に置く。媒体UIと動画レイアウトを混同しない。
...
動画作成画面の「訴求の自動構成」に対象者・悩み・解決策・証拠・CTAをすべて入力すると、Directorのfixtureに頼らず、次の順序を固定した `VideoPlan` を生成する。

`フック → 悩み → 解決 → 証拠 → CTA`

プロバイダーから直接使う場合は `composeCreativePlan` に同じ5項目とブランド、登録済み素材キーを渡す。字幕はブランドの上限内へ短縮し、素材は指定がなければ登録済みキーから決定的に選ぶ。入力が不足する場合は自動構成を開始せず、通常のテーマ生成へ戻せる。

冒頭比較には `composeCreativePlanVariants` を使う。同じブリーフから `question`（問いかけ）、`empathy`（共感）、`promise`（結果先出し）の3プランを作り、2秒時点の離脱や平均視聴時間を比較する。本文・証拠・CTAは同じにして、フックの差だけを評価する。
...
- シーン切替で全体を黒くするフェードは使わず、背景のパン・文字の登場・小さなスケール変化で連続性を保つ。暗転の点滅は視認性を下げるため避ける。
- キャラクター版は固定の基準画像、髪型・衣装・配色・持ち物・口調をIP設定として管理し、毎回ランダム生成しない。
- キャラクターを使わない比較版は `SHORTFACTORY_VIDEO_MODE=kinetic_text` で実行する。役割ごとに色・見出し・図形の動きが変わり、文字だけでも視線を誘導する。

文字訴求版の再現例：

```sh
SHORTFACTORY_VIDEO_MODE=kinetic_text \
SHORTFACTORY_BATCH_OUTPUT=/path/to/kinetic-text.mp4 \
pnpm --filter @shortfactory/video batch:settings
```
